...
public class HouseController : MonoBehaviour {
	...
	//Dijkstra's shortest path for gates!
	//https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm
	public List<Transform> GetShortestGatePath(Vector3 startPosition, Vector3 endPosition){

		List<Transform> gatePath = new List<Transform> ();

		//1. Find starting node, ending node, set all other node distances to infinity (incl. end node)
		Gate startPoint = HouseGates [0];
		startPoint.DijkstraDistance = 0;
		float minDistanceToStart = (startPosition - startPoint.transform.position).magnitude;

		Gate endPoint = HouseGates [0]; //don't set endpoint distance to -inf until later (though it should get set in looking for the start node anyway...)
		float minDistanceToEnd = (endPosition - endPoint.transform.position).magnitude;

		for (int i = 1; i < HouseGates.Count; i++) { //already used the 0 index to initialize, so start at index 1
			Gate currPoint = HouseGates[i];
			...
			if(startToCurrPointDist < minDistanceToStart){
				...
				HouseGates[i].DijkstraDistance = Mathf.Infinity;	//if it's not the shortest distance, set it's distance to infinity
			}
		...
		Gate currNode = startPoint;
		List<Gate> unvisitedPoints = new List<Gate> ();
		for (int i = 0; i < HouseGates.Count; i++) {
			if(HouseGates[i] != startPoint){
				unvisitedPoints.Add(HouseGates[i]);
			}
		}
		...
		while (unvisitedPoints.Count > 0) {
			...
			for (int i = 0; i < currNode.neighbors.Length; i++) {
				Gate currNeighbor = currNode.neighbors [i];
		...
		while(currNode != startPoint){
			gatePath.Add(currNode.transform);
		...
		if (gatePath.Count > 0) {
			nextPointPosition = gatePath[gatePath.Count - 1].transform.position;
		}
		if(!CheckBetweenPoints(startPosition, startPoint.transform.position, nextPointPosition)){
			gatePath.Add(startPoint.transform);
		}
